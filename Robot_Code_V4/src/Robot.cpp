 ...
 *
 *Hardware List:
 * -4 Spark Motor Controllers
 * -4 H-Bridge Spike Relays
 * -2 Limit Switches
 * -4 Joysticks(Driver Station)
 *
 * Comments next to code describe purpose of code if not used previously in the code (uncommented lines were automatically generated)
 * This robot code was made by/for FRC Team 522.
*/
...
public:
	...
	void AutonomousInit() override {
		...
		/*if (DriverStation::GetInstance().GetAlliance() == DriverStation::Alliance::kBlue){			//Check which alliance robot is on
			isBlue = true;																			//if we are on Blue alliance isBlue is true
		}
		else{
			isBlue = false;
		}*/
	...
	void AutonomousPeriodic() {
		if (Timer.Get()<2.0){																		//The Timer allows autonmous code to only execute once instead of looping
			...
			if(/*isBlue && */(m_autoSelected != kAutoNameDefault)){										//Are we on Blue Alliance? and is any non default autonomous selected?
			...
			/*else if (!(isBlue) && (m_autoSelected != kAutoNameDefault)){
				if (m_autoSelected == kDSRight){
					if (gameData[1] == 'R'){
				...
				else if (m_autoSelected == kDSLeft){
					if (gameData[1] == 'L') {
				...
				else if (m_autoSelected == kDSCenter){
					if (gameData[0] == 'L'){
...
private:
	...
	//Numbers in Brackets indicate port that each part is connected too (list of possible ports always start at 0)
